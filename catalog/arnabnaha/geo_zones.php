<?php
/*
$Id: geo_zones.php 1775 2012-04-01 19:13:57Z michael.oscmax@gmail.com $
...
*/
  ...
  if (tep_not_null($saction)) {
    switch ($saction) {
      case 'insert_sub':
        $zID = tep_db_prepare_input((int)$_GET['zID']);
      ...
      case 'save_sub':
        ...
        tep_redirect(tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . (int)$_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&sID=' . $_GET['sID']));
        break;
      case 'deleteconfirm_sub':
        ...
        tep_redirect(tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . (int)$_GET['zID'] . '&action=list&spage=' . $_GET['spage']));
        break;
    }
  }
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'insert_zone':
        ...
        tep_redirect(tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . (int)$new_zone_id));
        break;
      case 'save_zone':
        $zID = tep_db_prepare_input((int)$_GET['zID']);
        $geo_zone_name = tep_db_prepare_input($_POST['geo_zone_name']);
...
<?php
    ...
    while ($zones = tep_db_fetch_array($zones_query)) {
      ...
      if (isset($sInfo) && is_object($sInfo) && ($zones['association_id'] == $sInfo->association_id)) {
        echo '                  <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&amp;zID=' . (int)$_GET['zID'] . '&amp;action=list&amp;spage=' . $_GET['spage'] . '&amp;sID=' . $sInfo->association_id . '&amp;saction=edit') . '\'">' . "\n";
      } else {
        echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&amp;zID=' . (int)$_GET['zID'] . '&amp;action=list&amp;spage=' . $_GET['spage'] . '&amp;sID=' . $zones['association_id']) . '\'">' . "\n";
      }
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($sInfo) && is_object($sInfo) && ($zones['association_id'] == $sInfo->association_id)) { echo tep_image(DIR_WS_ICONS . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&amp;zID=' . (int)$_GET['zID'] . '&amp;action=list&amp;spage=' . $_GET['spage'] . '&amp;sID=' . $zones['association_id']) . '">' . tep_image(DIR_WS_ICONS . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" align="right"><?php echo $zones_split->display_links($zones_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['spage'], 'zpage=' . $_GET['zpage'] . '&amp;zID=' . (int)$_GET['zID'] . '&amp;action=list', 'spage'); ?></td>
                  </tr>
                </table></td>
              </tr>
              <tr>
                <td align="right" colspan="3"><?php if (empty($saction)) echo '<a href="' . tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&amp;zID=' . (int)$_GET['zID']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a> <a href="' . tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&amp;zID=' . (int)$_GET['zID'] . '&amp;action=list&amp;spage=' . $_GET['spage'] . '&amp;' . (isset($sInfo) ? 'sID=' . $sInfo->association_id . '&amp;' : '') . 'saction=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
            </table>
<?php
  } else {
...
<?php
    ...
    $zones_query = tep_db_query($zones_query_raw);
    while ($zones = tep_db_fetch_array($zones_query)) {
      if ((!isset($_GET['zID']) || (isset($_GET['zID']) && ($_GET['zID'] == (int)$zones['geo_zone_id']))) && !isset($zInfo) && (substr($action, 0, 3) != 'new')) {
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($zInfo) && is_object($zInfo) && ($zones['geo_zone_id'] == $zInfo->geo_zone_id)) { echo tep_image(DIR_WS_ICONS . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&amp;zID=' . $zones['geo_zone_id']) . '">' . tep_image(DIR_WS_ICONS . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
  ...
  if ($action == 'list') {
    ...
    switch ($action) {
      case 'new_zone':
        ...
        $contents = array('form' => tep_draw_form('zones', FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . (int)$_GET['zID'] . '&amp;action=insert_zone'));
