...
$PHPTHUMB_CONFIG['cache_directory'] = $PHPTHUMB_CONFIG['document_root']. DIR_WS_CATALOG .'cache/'; // set the cache directory to an absolute directory for all source images
...
$PHPTHUMB_CONFIG['cache_source_directory'] = DIR_WS_CATALOG .'cache/source/';  //absolute path
...
function phpThumbURL($ParameterString) {
    ...
    $strtr_slashes = array('\\'=>DIRECTORY_SEPARATOR, '/'=>DIRECTORY_SEPARATOR);
    return str_replace(strtr(@$PHPTHUMB_CONFIG['document_root'], $strtr_slashes), '', strtr(dirname(__FILE__), $strtr_slashes)).DIRECTORY_SEPARATOR.'phpThumb.php?'.$ParameterString.'&hash='.md5($ParameterString.@$PHPTHUMB_CONFIG['high_security_password']);
}
