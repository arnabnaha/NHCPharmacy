<?php
/*
$Id: rmairsure.php 1692 2012-02-26 01:26:50Z michael.oscmax@gmail.com $
...
class rmairsure {
	...
	function rmairsure() {
		...
		$this->version = ((defined('MODULE_SHIPPING_RMAIRSURE_VERSION')) ? MODULE_SHIPPING_RMAIRSURE_VERSION : '?.?');
		$this->code = 'rmairsure';
	...
	function install() {
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Version', 'MODULE_SHIPPING_RMAIRSURE_VERSION', '2.2.1', '', '6', '0', now())");
		...
		for ($i = 1; $i <= $this->num_zones; $i++) {
			...
			if ($i == 1) {
				...
				$shipping_table = '.1:6.21,.12:6.32,.14:6.47,.16:6.6,.18:6.75,.2:6.88,.22:7.02,.24:7.14,.26:7.27,.28:7.41,.3:7.55,.34:7.79,.38:8.03,.42:8.27,.46:8.51,.5:8.75,.56:9.08,.62:9.41,.68:9.74,.74:10.07,.8:10.95,.9:11.5,1:11.5,1.2:12.5,1.4:13.5,1.6:14.5,1.8:15.5,2:16.5';
				$shipping_compensation = '41:0,500:2.5';
				$shipping_handling = 0;
			...
			if ($i == 2) {
				...
				$shipping_table = '.1:6.72,.12:7,.14:7.28,.16:7.55,.18:7.83,.2:8.1,.22:8.36,.24:8.61,.26:8.86,.28:9.12,.3:9.37,.34:9.85,.38:10.33,.42:10.81,.46:11.29,.5:11.77,.56:12.43,.62:13.09,.68:13.75,.74:14.41,.8:16.17,.9:17.27,1:17.27,1.2:19.47,1.4:21.67,1.6:23.87,1.8:26.07,2:28.27';
				$shipping_compensation = '41:0,500:2.5';
				$shipping_handling = 0;
			// MODULE_SHIPPING_RMAIRSURE_COUNTRIES_$i
