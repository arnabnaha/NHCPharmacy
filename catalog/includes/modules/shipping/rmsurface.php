<?php
/*
$Id: rmsurface.php 1692 2012-02-26 01:26:50Z michael.oscmax@gmail.com $
...
class rmsurface {
	...
	function rmsurface() {
		...
		$this->version = ((defined('MODULE_SHIPPING_RMSURFACE_VERSION')) ? MODULE_SHIPPING_RMSURFACE_VERSION : '?.?');
		$this->code = 'rmsurface';
	...
	function install() {
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Version', 'MODULE_SHIPPING_RMSURFACE_VERSION', '2.2.1', 'Sort order of display (1 shown first 99 etc shown last to customer)', '6', '0', now())");
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Maximum value to ship', 'MODULE_SHIPPING_RMSURFACE_MAX_VALUE', '41', 'Enter the maximum value to ship', '6', '0', now())");
		...
		for ($i = 1; $i <= $this->num_zones; $i++) {
			...
			if ($i == 1) {
				...
				$shipping_table = '.1:0.99,.15:1.32,.2:1.66,.25:2,.3:2.32,.35:2.65,.4:3,.45:3.33,.5:3.65,.55:3.99,.6:4.33,.65:4.65,.7:4.99,.75:5.32,.8:5.66,.85:5.98,.9:6.32,.95:6.66,1:6.98,1.1:7.65,1.2:8.32,1.3:8.98,1.4:9.65,1.5:10.31,1.6:10.98,1.7:11.54,1.8:12.1,1.9:12.66,2:13.22';
				$shipping_handling = 0;
